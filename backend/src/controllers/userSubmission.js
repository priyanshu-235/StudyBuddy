...
const { getFirstSubmissionFailure, sendJudge0Error } = require("../utils/executionResponse");
...
const submitCode = async (req,res)=>{
    ...
    for(const test of testResult){
        if(test.status_id==3){
          ...
          if(!errorMessage){
            const failure = getFirstSubmissionFailure([test]);
            errorMessage = failure?.detail || failure?.explanation || test.stderr || null;
          }
        }
    }
    ...
    res.status(201).json({
      ...
      return sendJudge0Error(res, err);
    }
}
